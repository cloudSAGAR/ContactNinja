...
public class MyFancyCoverFlowAdapter extends FancyCoverFlowAdapter {
    ...
    public MyFancyCoverFlowAdapter(Context context, List<PlanItem> list) {
        mContext = context;
        this.list = list;
    }
    ...
    public View getCoverFlowItem(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(R.layout.plan_item_select, null);
            WindowManager wm = (WindowManager) mContext.getSystemService(Context.WINDOW_SERVICE);
            int width = wm.getDefaultDisplay().getWidth();
            convertView.setLayoutParams(new FancyCoverFlow.LayoutParams(width / 3, FancyCoverFlow.LayoutParams.MATCH_PARENT));
